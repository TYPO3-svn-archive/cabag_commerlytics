...
class tx_yellowpay2commerce {
	...
	var $scriptRelPath = 'class.tx_yellowpay2commerce.php';	// Path to this script relative to the extension dir.
	var $extKey = 'yellowpay2commerce';		
	...
	function finishingFunction($config, $session, $basket, &$pObj) {
		...
		if($this->checkFromYellowpay()){
			$pObj->orderUid = $GLOBALS['TSFE']->fe_user->getKey('ses', 'orderUID');
			$pObj->conf['lockOrderIdInGenerateOrderIdYellowpay'] = 1;
			$pObj->conf['lockOrderIdInGenerateOrderId'] = 0;
			return true;
		}
		...
		if(!empty($paymentURL) && !empty($ext_conf['yellowpayPSPID'])) {
			...
			$orderUid = 0;
			if (isset($pObj->conf['lockOrderIdInGenerateOrderId']) && $pObj->conf['lockOrderIdInGenerateOrderId'] == 1) {
				$orderData = array();
				$now = time();
				...
				$pObj->orderUid = $orderUid;
			}

			$orderID = $ext_conf['yellowpayOrderIDprefix'].$this->getLatestOrderID($orderUid);
			$GLOBALS['TSFE']->fe_user->setKey('ses', 'orderID', $orderID);
			$GLOBALS['TSFE']->fe_user->setKey('ses', 'orderUID', $orderUid);
	...
	function sendToYellowpay($paymentURL, $paymentParams) {
		...
		if (is_array($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['yellowpay2commerce/class.tx_yellowpay2commerce.php']['preSendToYellowpay'])) {
			foreach($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['yellowpay2commerce/class.tx_yellowpay2commerce.php']['preSendToYellowpay'] as $classRef) {
				$hookObjectsArr[] = &t3lib_div::getUserObj($classRef);
			}
		}

		foreach($hookObjectsArr as $hookObj) {
			if (method_exists($hookObj, 'preSendToYellowpay')) {
				$hookObj->preSendToYellowpay($paymentURL, $paymentParams, $this);
			}
		}
